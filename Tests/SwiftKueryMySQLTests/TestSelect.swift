...
class TestSelect: MySQLTest {
    ...
    func testConcurrentSelectAndSelectFromMany() {
        performTest(asyncTasks: { connection in
            ...
            executeQuery(query: s4, connection: connection) { result, rows in
                ...
                XCTAssertEqual(rows?[0][0]as? String, "banana", "Wrong value in row 0 column 0: \(String(describing: rows?[0][0])) instead of banana")
                XCTAssertEqual(rows?[1][0]as? String, "apple", "Wrong value in row 1 column 0: \(String(describing: rows?[1][0])) instead of apple")
            }
            ...
            executeQuery(query: s4Raw, connection: connection) { result, rows in
                ...
                XCTAssertEqual(rows?[0][0]as? String, "ba", "Wrong value in row 0 column 0: \(String(describing: rows?[0][0])) instead of ba")
                XCTAssertEqual(rows?[1][0]as? String, "ap", "Wrong value in row 1 column 0: \(String(describing: rows?[1][0])) instead of ap")
            }
            ...
            executeQuery(query: s5, connection: connection) { result, rows in
                ...
                XCTAssertEqual(rows?.count, 2, "SELECT returned wrong number of rows: \(String(describing: rows?.count)) instead of 2")
                XCTAssertEqual(rows?[0][0]as? String, "banana", "Wrong value in row 0 column 0: \(String(describing: rows?[0][0])) instead of banana")
                XCTAssertEqual(rows?[1][0]as? String, "banana", "Wrong value in row 1 column 0: \(String(describing: rows?[1][0])) instead of banana")
            }
            ...
            let s9 = "Select * from \(packName(t.tableName)) where a IN ('apple', 'lalala')"
            ...
            executeRawQuery("CREATE TABLE " +  packName(t1.tableName) + " (a varchar(40), b integer)", connection: connection) { result, rows in
                XCTAssertEqual(result.success, true, "CREATE TABLE failed")
                XCTAssertNil(result.asError, "Error in CREATE TABLE: \(result.asError!)")
            }

            executeRawQuery("CREATE TABLE " +  packName(t2.tableName) + " (c varchar(40), b integer)", connection: connection) { result, rows in
                XCTAssertEqual(result.success, true, "CREATE TABLE failed")
                XCTAssertNil(result.asError, "Error in CREATE TABLE: \(result.asError!)")
            }

            executeRawQuery("CREATE TABLE " +  packName(t3.tableName) + " (d varchar(40), b integer)", connection: connection) { result, rows in
                XCTAssertEqual(result.success, true, "CREATE TABLE failed")
                XCTAssertNil(result.asError, "Error in CREATE TABLE: \(result.asError!)")
            }
