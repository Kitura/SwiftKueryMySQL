...
class TestJoin: MySQLTest {

    static var allTests: [(String, (TestJoin) -> () throws -> Void)] {
        return [
            ("testConcurrentJoinAndUnion", testConcurrentJoinAndUnion)
        ]
    }

    class MyTable1a: Table {
        let a = Column("a")
        let b = Column("b")

        let tableName = table1Join + "a"
    }

    class MyTable2a: Table {
        let c = Column("c")
        let b = Column("b")

        let tableName = table2Join + "a"
    }

    class MyTable3a: Table {
        let d = Column("d")
        let b = Column("b")

        let tableName = tableUnion + "a"
    }

    class MyTable1b: Table {
        let a = Column("a")
        let b = Column("b")

        let tableName = table1Join + "b"
    }

    class MyTable2b: Table {
        let c = Column("c")
        let b = Column("b")

        let tableName = table2Join + "b"
    }

    class MyTable3b: Table {
        let d = Column("d")
        let b = Column("b")

        let tableName = tableUnion + "b"
    }
    
    func testConcurrentJoinAndUnion() {
        performTest(asyncTasks: { connection in
            let myTable1 = MyTable1a()
            let myTable2 = MyTable2a()
            let myTable3 = MyTable3a()

            cleanUp(table: myTable1.tableName, connection: connection) { _ in }
